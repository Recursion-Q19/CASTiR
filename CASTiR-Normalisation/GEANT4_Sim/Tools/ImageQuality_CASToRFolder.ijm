...
Before running this macro in ImageJ, must change the path to the image "test_with_corr_v3_it15.img" in run. 
This image "test_with_corr_v3_it15.img" is to be replaced with the correct reference image in the final updated version of this macro.

Two images are provided for testing the macro.

This is a compact version of ImageQualityVersion2.ijm which outputs only three windows.
Full Description can be found in readme file.

*/

run("Raw...", "open=/home/sayyed/Documents/STIR_folder/ImageReconstruction/ImageReconstruction-master/CASToR/NEMA-InTheMiddle-Final/SeyyedTest/SeyyedTest_it5.img image=[32-bit Real] width=160 height=160 number=139 little-endian"); //loaded image, change the path to the image here.
CASToR_imageTitle = getTitle();
Dialog.create("Upload Raw Image"); 
Dialog.addMessage("Choose an image please");
Dialog.show();
path = File.openDialog("Select a File");  
run("Raw...", "open=path image=[32-bit Real] width=160 height=160 number=139 little-endian");			
STIR_imageTitle = getTitle();

//image dimensions are known from (CASToR/STIR) generated log file

PIXEL_PER_mm_Taxial = 2.5; 
PIXEL_PER_mm_Axial = 1.6; 
img_width  = 160;                     
img_height = 160;
img_length = 139;//SM
...
function plotProfiles(image1, image2)
	{
		...
		Dialog.addNumber(" Region-1", 33); // 33, 57, 81 will appear by default for now chosen arbitrarily, user can enter a different number too
		Dialog.addNumber(" Region-2", 57);		
		Dialog.addNumber(" Region-3", 81);	
		...
		RMSE_Results(image1, image2, frame_initial_2, frame_final_2, 60, 60, 40, 40);  //  X,Y,W,H may change
...
run("Enhance Contrast...", "saturated=0.35 normalize process_all"); //Added By Seyyed
	    ...
	    setLocation(0, 340, 1500, 450);  // (x ,y ,width, height) adjusts the location of open window on screen (can be changed as per screen size)
		
		fontSize = 14;
   		setColor("white"); 
   		setFont("SansSerif", fontSize);  //font style, font size
	    	Overlay.drawString("CASToR",5,20);  // draws string at specified location in active image at 5, 20
	    	Overlay.show;
	    	Overlay.drawString("STIR",5,170);
	    	Overlay.show;   
		CloseWindow("Stack1");	
...
function Uniformity(image)
	{
		...
		startFrame =  57; //floor(22.5*PIXEL_PER_mm) central 10mm uniform region which is 2.5mm below and above the hot and cold regions respectively.
 	...
 	// draw z-profile at that point
		...
		for(i = 1; i < 6; i++){
			...
			for( j=0; j < Array_name.length; j++){
				Array_name[j]= getResult("Mean"+i, (j + startFrame) + 1);	
			}
...
function SORvalues(input_arr,image) 
	{
		...
		*/
		...
		startFrame = 35; // chosen by just looking at frames
        	stopFrame =  nofFrames+startFrame;
...
function Two_SOR_ROI(ImageSOR,input_arr)// inputs-> an average image to create ROIs, output_arr-> (has mean, std from Uniformity function) to calculate SOR, %STD  	
	...
	//ROI over the hot and cold region are created and added to the existing  ROI with name "Cold1" and "Cold2" (air filled and water filled regions)
		...
		makeOval(56, 75, 10, 10);  //4 mm diameter,  half of the actual rod diameter
		roiManager("Add");
		makeOval(94, 75, 10, 10);
		roiManager("Add");
		run("Clear Results");
...
.
		...
		*/
		roiManager("reset");
		selectImage(img_AVG); 
		makeOval(96, 77, 5, 5); // 5,10,15,20,25 corresponds to 2 mm,4mm,6mm,8mm,10mm -which is twice the actual radius of the rods as specied by NEMA - 5 pixels = Pixel_per_mm_Taxial*2
		roiManager("add");
		makeOval(80, 91, 10, 10);
		roiManager("add");
		makeOval(58, 83, 15, 15);
		roiManager("add");
		makeOval(56, 60, 20, 20);
		roiManager("add");
		makeOval(73, 51, 25, 25);
		roiManager("add");		
...
function multiplot(region, std_arr, test_arr, MSE, RCS)
			{	
				...
				Plot.setLimits(0, 105, 0, 105); // Plot.setLimits xMin, xMax, yMin, yMax
				Plot.addLegend("stir \n castor");
				Plot.addText("MSE:\n"+MSE+"\n"+"ChiSquared/"+DF+":\n"+RCS, 0.5, 0.8);
				Plot.show();					
			}
...
function Rod_Uniformity(image)
	{
		...
		makeRectangle(83, 61, 5, 5);  // X,Y,W,H may change for 3 by 3 pixels region around the centre
		roiManager("Add");
		roiManager("Select", 0);
		roiManager("Multi Measure");
