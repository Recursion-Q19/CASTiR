...
*/
	...
	phi = (45. * G4UniformRand() + 67.5); // min + (max -min) * rand()
	u = G4UniformRand();

	radius = std::sqrt( (u*(std::pow(r_out, 2) - std::pow(r_in, 2))) + std::pow(r_in, 2) );

	x = radius * std::cos( phi*deg );
	y = radius * std::sin( phi*deg);
	// z = radius * std::cos( theta );
	z = 53.75 * ((2. * G4UniformRand()) - 1.);
	
	
	particleGun->SetParticlePosition(G4ThreeVector(x*mm, y*mm, z*mm));

	//========================
		// G4cout << "Gun Energy (PrimaryGeneratorAction) = " << particleGun->GetParticleEnergy()/keV << "keV" << G4endl;
	//========================
