...
{ 
  // G4int nEvnt = aRun->GetNumberOfEvent();
	...
	double sum = 0.0;
  	for(const auto& pair: dataManager->histmode){sum += pair.second;}
	std::cout << "Number of recorded events: " << sum << std::endl;
	std::cout << "Number of recorded lines: " << (dataManager->histmode).size() << std::endl;
		
  	unsigned long int num_records = (dataManager->histmode).size();
  	dataManager->WriteCASToRheader(num_records, false);
