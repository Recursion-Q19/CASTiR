...
/*
	if(pmmaBox != 0){delete pmmaBox; pmmaBox = 0;}
	if(logicPmmaBox != 0){delete logicPmmaBox; logicPmmaBox = 0;}
	if(physicsPmmaBox != 0){delete physicsPmmaBox; physicsPmmaBox = 0;}
...
/*
	solidAnnulus = new G4Tubs("sAnnulus", r_in, r_out, ((actual_det_l + 4)/2.)*mm, 0, twopi); //+4 for surrounding the active area
	...
	solidActive = new G4Tubs("sActive", r_in + 3.*mm, r_out - 3.*mm, ((actual_det_l - 0.4)/2.)*mm, 0, twopi); // -0.4 for active area to be surrounded
...
void DetectorConstruction::VisualizationAttributes()
{
	...
	logicPmmaBox->SetVisAttributes(VisAttActive);
