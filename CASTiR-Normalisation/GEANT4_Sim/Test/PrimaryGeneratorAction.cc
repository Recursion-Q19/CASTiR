...
/*
	G4double n1 = G4UniformRand();
	G4double x, y, z;
	G4double phi, costheta, u, theta, r;
	...
	if(n1<=0.25){
		particleGun->SetParticlePosition(G4ThreeVector((x+5.)*mm, y*mm, (25.+z)*mm));
	}else if(n1<=0.5){
		particleGun->SetParticlePosition(G4ThreeVector((x+10.)*mm, y*mm, (25.+z)*mm));
	}else if(n1<=0.75){
		particleGun->SetParticlePosition(G4ThreeVector((x+15.)*mm, y*mm, (25.+z)*mm));
	}else{
		particleGun->SetParticlePosition(G4ThreeVector((x+25.)*mm, y*mm, (25.+z)*mm));
	} 
...
//=========================================================================================
	...
	r = std::sqrt(68.*68. + (G4UniformRand()*(69.*69. - 68.*68.)));

	x = r * std::cos(theta*rad);
	y = r * std::sin(theta*rad);
	z = 105.1 * G4UniformRand(); 
	
	
	particleGun->SetParticlePosition(G4ThreeVector(x*mm, y*mm, (z-52.55)*mm)); 
...
//========================================================================================	
	...
	theta = twopi*G4UniformRand(); 
	r = std::sqrt(15.*15. + (G4UniformRand()*(20.*20. - 15.*15.)));
	...
	particleGun->SetParticlePosition(G4ThreeVector(x*mm, y*mm, (z-3.)*mm));
...
//===========================================================================
	...
	particleGun->SetParticlePosition(G4ThreeVector((x+25.)*mm, y*mm, (z+30.)*mm));
*/
