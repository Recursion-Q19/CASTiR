...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
	...
	//============================
	/*
		if (aTrack->GetParentID() == 0 && aTrack->GetDefinition()->GetParticleName() == "e+" && aTrack->GetCurrentStepNumber() == 1) 
		{
			G4double ekin = aStep->GetPreStepPoint()->GetKineticEnergy();
			G4cout << "Primary positron initial kinetic energy (SteppingAction) = " << ekin/keV << "keV" << G4endl;
		}
	*/
	//============================
	...
	{ 
		...
		if(postStepPV)
		{
			...
			if(postStepPVname.contains("pShield") || postStepPVname.contains("pShield_side_right") ||
			 postStepPVname.contains("pShield_side_left")  || dataManager->GetInvalidEventFlag() == 1)
			{ 
...
//G4cout << "Photon BBBBB" << G4endl;
				...
				if(mID1!=mID2 && dataManager->GetNewEventFlag()==0 && mID1!=1000 && mID2 !=1000)
				{
					...
					//if( mID1==0 || mID2==0 ){
						dataManager->hist_map(cID1, mID1, cID2, mID2);
						dataManager->WriteSTIRdata(cID1, mID1, cID2, mID2);
						// G4cout << i << ". Writing WWWWWWWW" << G4endl;
						// i++;
					//}
					cID1 = 0; mID1 = 1000; cID2=0; mID2=1000;
					//G4cout << "Doneeeeeeeeeeeeeee" << G4endl;
